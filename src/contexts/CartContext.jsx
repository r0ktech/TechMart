...
const cartReducer = (state, action) => {
  switch (action.type) {
    case 'ADD_ITEM': {
      const existing = state.items.find(item => item.id === action.payload.id && item.variantKey === action.payload.variantKey);
      if (existing) {
        return {
          ...state,
          items: state.items.map(item =>
            item.id === action.payload.id && item.variantKey === action.payload.variantKey
    ...
    case 'REMOVE_ITEM':
      return { ...state, items: state.items.filter(item => !(item.id === action.payload.id && item.variantKey === action.payload.variantKey)) };
...
export function CartProvider({ children }) {
  ...
  const removeItem = (id, variantKey = 'default') => dispatch({ type: 'REMOVE_ITEM', payload: { id, variantKey } });
  const updateQuantity = (id, variantKey, quantity) => dispatch({ type: 'UPDATE_QUANTITY', payload: { id, variantKey, quantity } });
  ...
  return (
    <CartContext.Provider value={{
      ...
      cartCount, subtotal, discount, delivery, total, isInCart
    }}>
      {children}
